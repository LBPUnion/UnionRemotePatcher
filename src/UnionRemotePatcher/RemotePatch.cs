...
namespace UnionRemotePatcher
{
    public class RemotePatch
    {
        ...
        public void PSNEBOOTRemotePatch(string ps3ip, string gameID, string serverURL, string user, string pass)
        {
            ...
            {
                FTPHelper.FTP_Download($"ftp://{ps3ip}/dev_hdd0/home/{currentUser}/exdata/act.dat", @$"data/act.dat", user, pass);

                foreach (string fileName in FTPHelper.FTP_ListDirectory($"ftp://{ps3ip}/dev_hdd0/home/{currentUser}/exdata/", user, pass))
                {
                    if (fileName.Contains(gameID))
                    {
                        FTPHelper.FTP_Download($"ftp://{ps3ip}/dev_hdd0/home/{currentUser}/exdata/{fileName}", @$"data/{fileName}", user, pass);
                    }
                }
            }
        ...
        public void DiscEBOOTRemotePatch(string ps3ip, string gameID, string serverURL, string user, string pass)
        {
            ...
            // Now we'll check and see if a backup exists on the server or not, if we don't have one on the server, then upload one
            if (!FTPHelper.FTP_CheckFileExists($"ftp://{ps3ip}/dev_hdd0/game/{gameID}/USRDIR/EBOOT.BIN.BAK", user, pass)) { FTPHelper.FTP_Upload(@$"eboot/{gameID}/original/EBOOT.BIN", $"ftp://{ps3ip}/dev_hdd0/game/{gameID}/USRDIR/EBOOT.BIN.BAK", user, pass); }

            // Check for keys in the data directory
            if (!File.Exists("data/keys")) { throw new FileNotFoundException("UnionRemotePatcher cannot find the keys, ldr_curves, or vsh_curves files required to continue. Please make sure you have copies of these files placed in the data directory where you found the executable to run UnionRemotePatcher. Without them, we can't patch your game."); }
            ...
            FTPHelper.FTP_Upload(@$"eboot/{gameID}/patched/EBOOT.BIN", $"ftp://{ps3ip}/dev_hdd0/game/{gameID}/USRDIR/EBOOT.BIN", user, pass);
        }
    }
}